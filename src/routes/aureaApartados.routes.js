...
router.get('/', async (req, res) => {
  const { cliente_id, estado, producto_id } = req.query;
  const { rows } = await pool.query(
    `SELECT a.id, a.folio, a.cliente_id, cl.nombre AS cliente_nombre, a.producto_id, p.nombre AS producto_nombre,
            a.cantidad, a.precio_total, a.monto_abonado, a.estado,
            a.usuario_id, us.nombre AS usuario_nombre, a.created_at, a.updated_at
     FROM aurea_apartados a
     JOIN aurea_clientes cl ON cl.id = a.cliente_id
     JOIN aurea_productos p ON p.id = a.producto_id
     LEFT JOIN usuarios us ON us.id = a.usuario_id
     WHERE ($1::uuid IS NULL OR a.cliente_id = $1::uuid)
       AND ($2::text IS NULL OR a.estado::text = $2)
       AND ($3::uuid IS NULL OR a.producto_id = $3::uuid)
     ORDER BY a.created_at DESC`,
    [cliente_id || null, estado || null, producto_id || null]
  );
  res.json(rows);
});
...
router.get('/:id', async (req, res) => {
  const apartadoResult = await pool.query(
    `SELECT a.id, a.folio, a.cliente_id, cl.nombre AS cliente_nombre, cl.telefono AS cliente_telefono,
            a.producto_id, p.nombre AS producto_nombre,
            a.cantidad, a.precio_total, a.monto_abonado, a.estado,
            a.usuario_id, us.nombre AS usuario_nombre, a.created_at, a.updated_at
     FROM aurea_apartados a
     JOIN aurea_clientes cl ON cl.id = a.cliente_id
  ...
  const abonos = await pool.query(
    `SELECT ab.id, ab.monto, ab.metodo, ab.usuario_id, us.nombre AS usuario_nombre, ab.created_at
     FROM aurea_apartado_abonos ab LEFT JOIN usuarios us ON us.id = ab.usuario_id
...
router.post('/', async (req, res) => {
  const { cliente_id, producto_id, cantidad, precio_total, anticipo, metodo } = req.body ?? {};
  ...
  try {
    ...
    const prod = await client.query(
      `SELECT stock_cantidad, stock_apartado FROM aurea_productos WHERE id = $1 FOR UPDATE`,
      [producto_id]
    );
    if (!prod.rows[0]) throw Object.assign(new Error('Producto no encontrado.'), { statusCode: 404 });
    const disponible = prod.rows[0].stock_cantidad - prod.rows[0].stock_apartado;
    if (disponible < cantidad) {
      throw Object.assign(new Error('No hay suficiente stock disponible para apartar.'), { statusCode: 409 });
    }

    await client.query(
      `UPDATE aurea_productos SET stock_apartado = stock_apartado + $1, updated_at = now() WHERE id = $2`,
      [cantidad, producto_id]
    );

    const apartado = await client.query(
      `INSERT INTO aurea_apartados (cliente_id, producto_id, cantidad, precio_total, monto_abonado, usuario_id)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING id, folio, cliente_id, producto_id, cantidad, precio_total, monto_abonado, estado, created_at`,
      [cliente_id, producto_id, cantidad, Number(precio_total), montoAnticipo, req.usuario.sub]
    );

    if (montoAnticipo > 0) {
      await client.query(
        `INSERT INTO aurea_apartado_abonos (apartado_id, monto, metodo, usuario_id) VALUES ($1, $2, $3, $4)`,
...
router.post('/:id/abonos', async (req, res) => {
  ...
  try {
    ...
    const apartadoResult = await client.query(`SELECT precio_total, monto_abonado, estado FROM aurea_apartados WHERE id = $1 FOR UPDATE`, [req.params.id]);
    const apartado = apartadoResult.rows[0];
    ...
    await client.query(`UPDATE aurea_apartados SET monto_abonado = $1::numeric, updated_at = now() WHERE id = $2`, [nuevoMontoAbonado, req.params.id]);
...
router.patch('/:id', async (req, res) => {
  ...
  try {
    ...
    const actual = await client.query(`SELECT * FROM aurea_apartados WHERE id = $1 FOR UPDATE`, [req.params.id]);
    ...
    await client.query(
      `UPDATE aurea_productos SET stock_apartado = GREATEST(stock_apartado - $1, 0), updated_at = now() WHERE id = $2`,
      [apartado.cantidad, apartado.producto_id]
    );
    ...
    if (estado === 'completado') {
      ...
      await client.query(
        `INSERT INTO aurea_movimientos_inventario (producto_id, tipo, cantidad, motivo, referencia_tipo, referencia_id, usuario_id)
         VALUES ($1, 'salida', $2, 'Apartado completado — cliente se lleva el producto', 'apartado', $3, $4)`,
        [apartado.producto_id, apartado.cantidad, apartado.id, req.usuario.sub]
      );
    }

    const { rows } = await client.query(
      `UPDATE aurea_apartados SET estado = $1, updated_at = now() WHERE id = $2
       RETURNING id, folio, cliente_id, producto_id, cantidad, precio_total, monto_abonado, estado, updated_at`,
